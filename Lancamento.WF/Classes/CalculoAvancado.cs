...
namespace Lancamento.WF
{
    internal class CalculoAvancado
    {
        ...
        public static double Cos(double x, double precisaoDesejada = 1e-20)// Função que calcula valor de cos(x) baseado na serie de McLaurin
        {
            double f = 0;
            int k = 0;
            int cont = 0;
        ...
        public static double Sen(double x, double precisaoDesejada = 1e-20)
        {
            ...
            while (true)
            {
                if (k % 2 == 0)
                {
                    f += (Math.Pow(-1, cont) / CalculoAvancado.Fatorial(2 * cont + 1)) * Math.Pow(x, 2 * cont + 1); // Valor genérico da função sen(x) em MacLaurin                   
                    cont += 1;
                }
                Resto = (1 / CalculoAvancado.Fatorial(k + 1)) * Math.Pow(Math.Abs(x), k + 1);
                if (Resto <= precisaoDesejada)
